...
def make_modpath_model(modelpth="./", thisModelName="MONTAGUE_drb1.04_mf6_250", flow_model_name = "MONTAGUE_drb1.04_mf6_250", totalParts = 3e6):
    ...
    for thisPer in range(len(rech)):
        ...
        for thisSqr in sqrList:
            ...
            pg1 = fp.modpath.ParticleGroupNodeTemplate(particlegroupname=nameField, particledata=p, filename=os.path.join("arrays","%s.txt"%nameField), releasedata=[thisPer*perLen+perLen/2])
    ...
    mpsim = fp.modpath.Modpath7Sim(mp,
                               ...
                               weaksinkoption="pass_through" if weak_pass else 'stop_at',
                               weaksourceoption="pass_through" if weak_pass else 'stop_at',
                               budgetoutputoption='summary',
                               simulationtype = 'timeseries',
                               stoptimeoption='extend',
                               zonedataoption='on',
                               timepointdata = [len(timePtList),timePtList] if maxDepth_Calc else None,
                               stopzone=-1,
